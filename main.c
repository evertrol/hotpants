...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    if (fits_create_file(&oPtr, scrStr, &status) || fits_create_img(oPtr, oBitpix, oNaxis, oNaxes, &status)) {
        printError(status);
    }
    ...
    thetime = time(NULL);
    tm = gmtime(&thetime);
    sprintf(hInfo, "%04d-%02d-%02dT%02d:%02d:%02d", tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour,
            tm->tm_min, tm->tm_sec);
    ...
    if (convImage) {
        ...
        if (fits_create_file(&ePtr, scrStr, &status) || fits_create_img(ePtr, oBitpix, oNaxis, oNaxes, &status) ||
    ...
    if (outMask) {
        ...
        if (fits_create_file(&ePtr, scrStr, &status) || fits_create_img(ePtr, SHORT_IMG, oNaxis, oNaxes, &status) ||
            hp_fits_copy_header(iPtr, ePtr, &status) ||
            fits_update_key(ePtr, TSTRING, "OBJECT", "Hotpants output Mask Image", "", &status) ||
            fits_update_key_flt(ePtr, "BZERO", 32768, -5, "", &status) ||
            fits_update_key_flt(ePtr, "BSCALE", 1, -5, "", &status) || fits_close_file(ePtr, &status)) {
            printError(status);
        }
    }
    ...
    if (useFullSS) {
        ...
        fwStamp = imin(imin(tNaxes[0], iNaxes[0]) / nRegX / nStampX, imin(tNaxes[1], iNaxes[1]) / nRegY / nStampY);
        fwStamp -= fwKernel;
    ...
    if (!(temp = (float *)calloc((fwKSStamp + fwKernel) * fwKSStamp, sizeof(float))) ||
        ...
        !(check_mat = (double **)calloc(nC, sizeof(double *))) || !(check_vec = (double *)calloc(nC, sizeof(double))) ||
    ...
    if (regFile) {
        ...
        if (!(rXMins = (int *)calloc(nR, sizeof(int))) || !(rXMaxs = (int *)calloc(nR, sizeof(int))) ||
            !(rYMins = (int *)calloc(nR, sizeof(int))) || !(rYMaxs = (int *)calloc(nR, sizeof(int)))) {
            exit(1);
        }
    ...
    if ((doKerInfo) || (kernelImOut)) {
        ...
        if (kernelImOut) {
            ...
            if (fits_create_file(&ePtr, scrStr, &status) ||
                ...
                fits_get_hdu_num(ePtr, &kInfoNum) || fits_close_file(ePtr, &status)) {
                printError(status);
            }
        }
    }
    ...
    for (i = 0; i < nR; i++) {
        if (kernelImIn) {
            ...
            readKernel(kernelImIn, i, &tKerSol, &iKerSol, &rXMin, &rXMax, &rYMin, &rYMax, &meansigSubstamps,
                       &scatterSubstamps, &meansigSubstampsF, &scatterSubstampsF, &diffrat, &NskippedSubstamps);
        } else {
            rXMin = rXMins[i];
            rXMax = rXMaxs[i];
            rYMin = rYMins[i];
        ...
        */
                        ...
                        if (sstampFile) {
                            ...
                            for (m = 0; m < Ncmp; m++) {
                                if ((xcmp[m] > sXMin + hwKernel + 1) && (xcmp[m] < sXMax - hwKernel - 1) &&
                                    ...
                                    buildStamps(sXMin, sXMax, sYMin, sYMax, &niS, &ntS, 0, rXBMin, rYBMin, ciStamps,
                                                ctStamps, iRData, tRData, xcmp[m] - rXBMin, ycmp[m] - rYBMin);
                                }
                            }
                            if (findSSC) {
                                if (verbose >= 2) {
                                    fprintf(stderr, "Automatically finding additional centers\n");
                                }
                                /* and then choose enough stamps manually to fill the array */
                                buildStamps(sXMin, sXMax, sYMin, sYMax, &niS, &ntS, 1, rXBMin, rYBMin, ciStamps,
        ...
        /* perhaps for good! */
        if ((strncmp(forceConvolve, "t", 1) == 0) || ((tMerit < iMerit) && (!(strncmp(forceConvolve, "i", 1) == 0)))) {
            convTmpl = 1;
        ...
        */
        ...
        if (inclConvImage || convImage) {
            ...
            for (l = hwKernel; l < rPixY - hwKernel; l++) {
                for (k = hwKernel; k < rPixX - hwKernel; k++) {
                    if ((strncmp(photNormalize, "u", 1) != 0) && ((convTmpl && strncmp(photNormalize, "t", 1) == 0) ||
            ...
            if (inclConvImage) {
                ...
                if (hp_fits_write_subset(oPtr, 0, 2, oNaxes, oRData, &status, outShort, outBzero, outBscale, fpixelOutX,
            ...
            if (convImage) {
                ...
                if (hp_fits_write_subset(ePtr, 0, 2, oNaxes, oRData, &status, outShort, outBzero, outBscale, fpixelOutX,
                                         fpixelOutY, lpixelOutX, lpixelOutY, xBufLo, yBufLo) ||
                    fits_close_file(ePtr, &status)) {
                    printError(status);
                }
        ...
        getStampStats3(oRData, 0, 0, rPixX, rPixY, &sum, &mean, &median, &mode, &sd, &fwhm, &lfwhm, 0x0, 0xffff, 5);
        ...
        if (convTmpl) {
            getStampStats3(tRData, 0, 0, rPixX, rPixY, &nsum, &nmean, &nmedian, &nmode, &nsd, &nfwhm, &nlfwhm, 0x0,
                           0xffff, 5);
            getNoiseStats3(oRData, tRData, &x2norm, &nx2norm, 0x0, 0xffff);
        }

        else {
            getStampStats3(iRData, 0, 0, rPixX, rPixY, &nsum, &nmean, &nmedian, &nmode, &nsd, &nfwhm, &nlfwhm, 0x0,
                           0xffff, 5);
        ...
        getStampStats3(oRData, 0, 0, rPixX, rPixY, &summ, &meanm, &medianm, &modem, &sdm, &fwhmm, &lfwhmm, 0xff,
                       FLAG_OUTPUT_ISBAD, 5);
        fprintf(stderr, "   Mean   : %.2f\n", meanm);
        fprintf(stderr, "   Median : %.2f\n", medianm);
        fprintf(stderr, "   Mode   : %.2f\n", modem);
        ...
        if (convTmpl) {
            getStampStats3(tRData, 0, 0, rPixX, rPixY, &nsumm, &nmeanm, &nmedianm, &nmodem, &nsdm, &nfwhmm, &nlfwhmm,
                           0xff, FLAG_OUTPUT_ISBAD, 5);
        } else {
            getStampStats3(iRData, 0, 0, rPixX, rPixY, &nsumm, &nmeanm, &nmedianm, &nmodem, &nsdm, &nfwhmm, &nlfwhmm,
                           0xff, FLAG_OUTPUT_ISBAD, 5);
        }
        ...
        if (fits_movabs_hdu(oPtr, 1, NULL, &status) ||
            hp_fits_write_subset(oPtr, 0, 2, oNaxes, oRData, &status, outShort, outBzero, outBscale, fpixelOutX,
                                 fpixelOutY, lpixelOutX, lpixelOutY, xBufLo, yBufLo)) {
            printError(status);
        }
        ...
        if (inclNoiseImage) {
            if (fits_movrel_hdu(oPtr, 1, NULL, &status) || fits_set_bscale(oPtr, outNiBscale, outNiBzero, &status)) {
                printError(status);
            }

            if (convTmpl) {
                if (hp_fits_write_subset(oPtr, 0, 2, oNaxes, tRData, &status, outNShort, outNiBzero, outNiBscale,
                                         fpixelOutX, fpixelOutY, lpixelOutX, lpixelOutY, xBufLo, yBufLo)) {
                    printError(status);
                }
            } else {
                if (hp_fits_write_subset(oPtr, 0, 2, oNaxes, iRData, &status, outNShort, outNiBzero, outNiBscale,
                                         fpixelOutX, fpixelOutY, lpixelOutX, lpixelOutY, xBufLo, yBufLo)) {
                    printError(status);
                }
            }
        ...
        if (noiseImage) {
            ...
            if (convTmpl) {
                if (hp_fits_write_subset(ePtr, 0, 2, oNaxes, tRData, &status, outNShort, outNiBzero, outNiBscale,
                                         fpixelOutX, fpixelOutY, lpixelOutX, lpixelOutY, xBufLo, yBufLo)) {
                    printError(status);
                }
            } else {
                if (hp_fits_write_subset(ePtr, 0, 2, oNaxes, iRData, &status, outNShort, outNiBzero, outNiBscale,
                                         fpixelOutX, fpixelOutY, lpixelOutX, lpixelOutY, xBufLo, yBufLo)) {
                    printError(status);
                }
            }
            if (fits_write_key_str(ePtr, hKeyword, hInfo, "", &status) || fits_close_file(ePtr, &status)) {
                printError(status);
            }
        }
        ...
        if (outMask) {
            ...
            if (hp_fits_write_subset_int(ePtr, 0, 2, oNaxes, mRData, &status, 1, 32768, 1, fpixelOutX, fpixelOutY,
        ...
        if (kernelImOut) {
            if (convTmpl) {
                if (fits_open_file(&ePtr, kernelImOut, 1, &status) || fits_movabs_hdu(ePtr, kInfoNum, NULL, &status) ||
