    ...
    for (j = -hwKSStamp; j <= hwKSStamp; j++) {
        for (i = -hwKSStamp; i <= hwKSStamp; i++) {
            ...
            for (xc = -hwKernel; xc <= hwKernel; xc++) {
                imc[xij] +=
                  temp[i + xc + sub_width / 2 + sub_width * (j + hwKSStamp)] * filter_x[hwKernel - xc + n * fwKernel];
            }
        }
    }
    ...
    for (j = yi - hwKSStamp; j <= yi + hwKSStamp; j++) {
        for (i = xi - hwKSStamp; i <= xi + hwKSStamp; i++) {
            ...
            for (yc = -hwKernel; yc <= hwKernel; yc++) {
                for (xc = -hwKernel; xc <= hwKernel; xc++) {
                    imc[xij] +=
                      image[(i + xc) + rPixX * (j + yc)] * PCA[n][(xc + hwKernel) + fwKernel * (yc + hwKernel)];
                }
            }
        }
    }
...
void fitKernel(stamp_struct *stamps, float *imRef, float *imConv, float *imNoise, double *kernelSol,
    ...
    check =
      check_again(stamps, kernelSol, imConv, imRef, imNoise, meansigSubstamps, scatterSubstamps, NskippedSubstamps);
    ...
    if ((strncmp(forceConvolve, "b", 1) == 0)) {
        ...
        for (i = 0; i < nS; i++) {
            if (stamps[i].diff < kerSigReject) {
                ...
                if (verbose >= 2) {
                    fprintf(stderr, "    # %d    skipping xss: %4i yss: %4i ksum: %f sigma: %f\n", i,
                            stamps[i].xss[stamps[i].sscnt], stamps[i].yss[stamps[i].sscnt], stamps[i].norm,
                            stamps[i].diff);
                }
            }
        }
...
void getStampSig(stamp_struct *stamp, double *kernelSol, float *imNoise, double *sig1, double *sig2, double *sig3) {
    ...
    *sig3 = 0;
    ...
    if (strncmp(figMerit, "v", 1) != 0) {
        if (getStampStats3(temp, xRegion - hwKSStamp, yRegion - hwKSStamp, fwKSStamp, fwKSStamp, &cSum, &cMean,
                           &cMedian, &cMode, sig2, sig3, &cLfwhm, 0x0, 0xffff, 5)) {
            *sig2 = -1;
            *sig3 = -1;
...
char check_again(stamp_struct *stamps, double *kernelSol, float *imConv, float *imRef, float *imNoise,
                 double *meansigSubstamps, double *scatterSubstamps, int *NskippedSubstamps) {
    /*****************************************************
    ...
    *NskippedSubstamps = 0;
    ...
    for (istamp = 0; istamp < nS; istamp++) {
        ...
        if (stamps[istamp].sscnt < stamps[istamp].nss) {
            ...
            if ((strncmp(figMerit, "v", 1) == 0 && (sig1 == -1)) || (strncmp(figMerit, "s", 1) == 0 && (sig2 == -1)) ||
                ...
                if (verbose >= 2) {
                    fprintf(stderr,
                            ...
                            istamp, stamps[istamp].xss[stamps[istamp].sscnt], stamps[istamp].yss[stamps[istamp].sscnt],
                ...
                if (verbose >= 2) {
                    fprintf(stderr, "    # %d    xss: %4i yss: %4i sig: %6.3f sscnt: %2i nss: %2i OK\n", istamp,
                            stamps[istamp].xss[stamps[istamp].sscnt], stamps[istamp].yss[stamps[istamp].sscnt], sig,
            ...
            if ((stamps[istamp].chi2 - mean) > kerSigReject * stdev) {
                if (verbose >= 2) {
                    fprintf(
                      ...
                      istamp, stamps[istamp].xss[stamps[istamp].sscnt], stamps[istamp].yss[stamps[istamp].sscnt],
                      stamps[istamp].chi2, stamps[istamp].sscnt, stamps[istamp].nss);
                }
...
void spatial_convolve(float *image, float **variance, int xSize, int ySize, double *kernelSol, float *cRdata,
                      int *cMask) {
    /*****************************************************
