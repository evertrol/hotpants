...
int main(int argc, char **argv) {
    ...
    snprintf(help, NHELP,
             "Usage : extractkern [options] diffimage outimage\n"
             "Options:\n"
    ...
    if (inConv) {
        if (fits_open_file(&fPtr, inConv, 0, &status) || fits_get_img_param(fPtr, 2, NULL, NULL, cNaxes, &status)) {
            printError(status);
        }
    ...
    if (fits_create_file(&fPtr, scrStr, &status) || fits_create_img(fPtr, FLOAT_IMG, 2, oNaxes, &status) ||
        fits_write_img_flt(fPtr, 1, 1, xsize * ysize, dconv, &status) || fits_close_file(fPtr, &status)) {
        printError(status);
    }
...
void getKernel(char *kimage, double **kerSol) {
    ...
    for (i = 0; i < nR; i++) {
        ...
        if (((xConv > -1) && (yConv > -1)) &&
            ((xConv + 1) >= rXMin && (xConv + 1) <= rXMax && (yConv + 1) >= rYMin && (yConv + 1) <= rYMax)) {
