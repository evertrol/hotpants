...
void loadxyfile(char *filename, int cmpfileflag) {
    ...
    Nalloc = 100;
    if (!(xcmp = (float *)malloc(Nalloc * sizeof(float))) || !(ycmp = (float *)malloc(Nalloc * sizeof(float)))) {
        exit(1);
    }
    Ncmp = 0;
...
void savexy(stamp_struct *stamps, int nStamps, long xmin, long ymin, int regioncounter) {
    ...
    for (istamp = 0; istamp < nStamps; istamp++) {
        for (sscnt = 0; sscnt < stamps[istamp].nss; sscnt++) {
            /*fprintf(xyfileall,        " %4ld %4ld\n", stamps[istamp].xss[sscnt] + xmin, stamps[istamp].yss[sscnt] +
             * ymin);*/
...
void buildStamps(int sXMin, int sXMax, int sYMin, int sYMax, int *niS, int *ntS, int getCenters, int rXBMin, int rYBMin,
                 stamp_struct *ciStamps, stamp_struct *ctStamps, float *iRData, float *tRData, float hardX,
                 float hardY) {
    ...
    if (!(strncmp(forceConvolve, "i", 1) == 0)) {
        ...
        if (ctStamps[*ntS].nss == 0) {
            ...
            cutStamp(tRData, refArea, rPixX, sXMin - rXBMin, sYMin - rYBMin, sXMax - rXBMin, sYMax - rYBMin,
                     &ctStamps[*ntS]);

            if (!(getStampStats3(refArea, ctStamps[*ntS].x0, ctStamps[*ntS].y0, sPixX, sPixY, &ctStamps[*ntS].sum,
                                 &ctStamps[*ntS].mean, &ctStamps[*ntS].median, &ctStamps[*ntS].mode, &ctStamps[*ntS].sd,
                                 &ctStamps[*ntS].fwhm, &ctStamps[*ntS].lfwhm, 0x0, 0xffff, 3))) {
                /* pointless */
                if (verbose >= 1) {
                    fprintf(stderr, "    Tmpl  xs : %4i ys : %4i  (sky,dsky = %.1f,%.1f)\n", ctStamps[*ntS].x,
                            ctStamps[*ntS].y, ctStamps[*ntS].mode, ctStamps[*ntS].fwhm);
                }
            }
            free(refArea);
        }
    ...
    if (!(strncmp(forceConvolve, "t", 1) == 0)) {
        ...
        if (ciStamps[*niS].nss == 0) {
            ...
            cutStamp(iRData, refArea, rPixX, sXMin - rXBMin, sYMin - rYBMin, sXMax - rXBMin, sYMax - rYBMin,
                     &ciStamps[*niS]);

            if (!(getStampStats3(refArea, ciStamps[*niS].x0, ciStamps[*niS].y0, sPixX, sPixY, &ciStamps[*niS].sum,
                                 &ciStamps[*niS].mean, &ciStamps[*niS].median, &ciStamps[*niS].mode, &ciStamps[*niS].sd,
                                 &ciStamps[*niS].fwhm, &ciStamps[*niS].lfwhm, 0x0, 0xffff, 3))) {

                /* pointless */
                /* buildSigMask(&ciStamps[*niS], sPixX, sPixY, misRData); */
                if (verbose >= 1) {
                    fprintf(stderr, "    Image xs : %4i ys : %4i  (sky,dsky = %.1f,%.1f)\n", ciStamps[*niS].x,
                            ciStamps[*niS].y, ciStamps[*niS].mode, ciStamps[*niS].fwhm);
                }
            }
            free(refArea);
        }
        ...
        if (getCenters) {
            ...
            if (nss < nKSStamps) {
                ...
                check = checkPsfCenter(tRData, xmax - ctStamps[*ntS].x0, ymax - ctStamps[*ntS].y0, sPixX, sPixY,
                                       ctStamps[*ntS].x0, ctStamps[*ntS].y0, tUKThresh, ctStamps[*ntS].mode,
                                       1. / ctStamps[*ntS].fwhm, 0, 0, bbitt1 | bbitt2 | 0xbf, bbitt1);
        ...
        if (getCenters) {
            ...
            if (nss < nKSStamps) {
                ...
                check = checkPsfCenter(iRData, xmax - ciStamps[*niS].x0, ymax - ciStamps[*niS].y0, sPixX, sPixY,
                                       ciStamps[*niS].x0, ciStamps[*niS].y0, iUKThresh, ciStamps[*niS].mode,
                                       1. / ciStamps[*niS].fwhm, 0, 0, bbiti1 | bbiti2 | 0xbf, bbiti1);
...
void cutStamp(float *data, float *refArea, int dxLen, int xMin, int yMin, int xMax, int yMax, stamp_struct *stamp) {
...
int cutSStamp(stamp_struct *stamp, float *iData) {
    ...
    if (sscnt >= nss) {
        if (verbose >= 2) {
            fprintf(stderr,
                    "    xs : %4i ys : %4i sig: %6.3f sscnt: %4i nss: %4i ******** REJECT stamp (out of substamps)\n",
...
double checkPsfCenter(float *iData, int imax, int jmax, int xLen, int yLen, int sx0, int sy0, double hiThresh,
                      float sky, float invdsky, int xbuffer, int ybuffer, int bbit, int bbit1) {
...
int getPsfCenters(stamp_struct *stamp, float *iData, int xLen, int yLen, double hiThresh, int bbit1, int bbit2) {
    ...
    */
        ...
        for (j = ybuffer; j < yLen - ybuffer; j++) {
            ...
            for (i = xbuffer; i < xLen - xbuffer; i++) {
                ...
                if (dpt > loPsf) {
                    ...
                    dmax2 = checkPsfCenter(iData, imax, jmax, xLen, yLen, sx0, sy0, hiThresh, sky, invdsky, xbuffer,
                                           ybuffer, bbit, bbit1);
    ...
    if (pcnt + stamp->nss < nKSStamps) {
        if (verbose >= 2) {
            fprintf(stderr, "    ...only found %d good substamps by autosearch\n", pcnt);
        }
    } else {
        if (verbose >= 2) {
            fprintf(stderr, "    ...found %d good substamps by autosearch\n", pcnt);
        }
    }
    ...
    if (pcnt == 0) {
        ...
        if (verbose >= 2) {
            fprintf(stderr, "    NO good pixels, skipping...\n");
        }
    ...
    */
    ...
    for (i = rPixX * rPixY; i--;) {
        ...
        */
        ...
        if (((umask > 0) && (!(mdat & umask))) || ((smask > 0) && (mdat & smask)) || (fabs(ddat) <= ZEROVAL)) {
            continue;
        }
...
int getStampStats3(float *data, int x0Reg, int y0Reg, int nPixX, int nPixY, double *sum, double *mean, double *median,
                   double *mode, double *sd, double *fwhm, double *lfwhm, int umask, int smask, int maxiter) {
    /*****************************************************
    ...
    */
    ...
    if (!(sdat = (float *)calloc(npts, sizeof(float))) || !(work = (double *)calloc(nstat, sizeof(double)))) {
        return (1);
    }
    ...
    for (i = 0; (i < nstat) && (goodcnt < npts); i++, goodcnt++) {
        ...
        if (((umask > 0) && (!(mdat & umask))) || ((smask > 0) && (mdat & smask)) || (fabs(rdat) <= ZEROVAL)) {
            i--;
        } else {
            work[i] = rdat;
        }
...
void freeStampMem(stamp_struct *stamps, int nStamps) {
    ...
    if (stamps) {
        for (i = 0; i < nStamps; i++) {
            for (j = 0; j < nCompKer + nBGVectors; j++) {
                if (stamps[i].vectors[j]) {
                    free(stamps[i].vectors[j]);
                }
            }
            if (stamps[i].vectors) {
                free(stamps[i].vectors);
            }
...
void getKernelInfo(char *kimage) {
    ...
    *****************************************************/
    ...
    if (fits_get_num_hdus(kPtr, &existsTable, &status) || fits_movabs_hdu(kPtr, existsTable, NULL, &status) ||
...
void readKernel(char *kimage, int nRegion, double **tKerSol, double **iKerSol, int *rXMin, int *rXMax, int *rYMin,
                int *rYMax, double *meansigSubstamps, double *scatterSubstamps, double *meansigSubstampsF,
                double *scatterSubstampsF, double *diffrat, int *NskippedSubstamps) {
...
#define FSTRNCMP(a, b, n) ((a)[0] < (b)[0] ? -1 : (a)[0] > (b)[0] ? 1 : strncmp((a), (b), (n)))
    ...
    if (fits_get_hdrspace(iPtr, &nkeys, NULL, status) || fits_read_key_lng(iPtr, "NAXIS", &naxis, NULL, status)) {
        return *status;
    }
...
int hp_fits_write_subset(fitsfile *fptr, long group, long naxis, long *naxes, float *data, int *status, int makeShort,
                         float bZero, float bScale, int fpixelX, int fpixelY, int lpixelX, int lpixelY, int xArrayLo,
                         int yArrayLo) {
...
int hp_fits_write_subset_int(fitsfile *fptr, long group, long naxis, long *naxes, int *data, int *status, int makeShort,
                             float bZero, float bScale, int fpixelX, int fpixelY, int lpixelX, int lpixelY,
                             int xArrayLo, int yArrayLo) {
...
void quick_sort_1(double *list, int *index, int left_end, int right_end) {
    ...
    if (left_end < j) {
        quick_sort_1(list, index, left_end, j);
    }
    if (i < right_end) {
        quick_sort_1(list, index, i, right_end);
    }
